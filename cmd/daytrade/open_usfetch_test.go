...
func TestUsFetchLimits(t *testing.T) {
	...
	cases := []struct {
		...
		{"残り 8 秒: 取得に使えるのは 5 秒", true, now.Add(8 * time.Second), 3 * time.Second, 2 * time.Second},
		{"残り 5 秒: 取得に使えるのは 2 秒", true, now.Add(5 * time.Second), 2 * time.Second, time.Millisecond},
		{"残りが予約より短い: キャッシュだけ", true, now.Add(2 * time.Second), time.Millisecond, time.Millisecond},
