...
`wbjp evaluate` は `--date` を省くと、**まだ評価していない判断日**（同じ `--horizon` で実績の
出た評価が無い日）を古い順に全部評価する。`--horizon` 営業日後の足が揃っていない日は
何も書かずに飛ばし、揃った晩に評価される。同じ判断日を評価し直しても `wbjp review` は
（判断日, horizon）ごとに最後の評価だけを数える。
