...
**寄る前の `snap` は 8:59:43 の 1 回**（crontab の `DT_PRESNAP_AT`、`--slot 085943`、`--max-run 5` / 打ち切り 8 秒 / KILL 1 秒。
2026-09-24 に 8:59:35・8:59:55 の 2 回からまとめた）。寄る前の `open` の**前**に全銘柄（実測 起動 1.8 秒 + 取得 3.3〜3.6 秒）を
取り切り、固まっても 8:59:52 にはロックを手放す——`DT_PRESNAP_AT + 打ち切り + KILL ≤ DT_PREOPEN_AT − 1 秒` を組で保つ。
...
`daytrade snap`（8:30・8:45・8:55・8:57・8:59:00・8:59:43／9:00:05・9:02・9:05・9:11／15:00・15:19）は板・気配を記録するだけで
...
ふだんの実行は長くても 10 秒。過ぎたら TERM、さらに 10 秒（`WITH_LOCK_KILL_AFTER`。8:59:43 の `snap` は 1 秒）で KILL し、`[error] [timeout]` をログに残して 124 で終わる。
...
**既定（170 + 10）のままでよいのは、TERM と KILL が 8:59:53 の寄成（と 8:59:43 の snap）より手前に着く行だけ。** 8:30・8:45・8:55 の `snap` は
