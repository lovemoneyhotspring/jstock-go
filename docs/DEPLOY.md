...
「寄る前に出す」。実測 4〜9 秒で終わるので `WITH_LOCK_TIMEOUT` は 18 秒（8:59:48 起動 + 18 = 9:00:06。起動 + 打ち切り ≥ 9:00:05 を保つ）。

**寄る前の全銘柄の板は、寄る前の `open` の中で撮る**（2026-09-25〜。`open --presnap-until $DT_PRESNAP_UNTIL --quotes-at $DT_QUOTES_AT`）。
`open` は 8:59:48 に起動し、候補の外の板を `DT_PRESNAP_UNTIL`（いま 8:59:51.2）まで撮って `history/book` に残し（slot は撮り始めた時刻。起動の約 1 秒後で `085949` など）、
...
`daytrade snap`（8:57・8:59:00・8:59:30／9:00:05・9:02・9:05・9:11／15:00・15:19）は板・気配を記録するだけで
...
**既定（170 + 10）のままでよいのは、TERM と KILL が寄る前の寄成（`DT_PREOPEN_AT`、8:59:48）より手前に着く行だけ（2026-09-25 から寄る前の全銘柄の板は `open` の中で撮る）。** 8:57 の回は
別行に出して 60 + 5 に詰めてある（8:30・8:45・8:55 の回は 2026-09-25 にやめた）（既定だと KILL が 09:00:01 に当たり、
