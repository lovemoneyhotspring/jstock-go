...
**寄る前の `snap` は 1 回**（crontab の `DT_PRESNAP_AT`、いま 08:59:42.0・`--slot 085942`、`--max-run 5` / 打ち切り 8 秒 / KILL 1 秒。
