...
- `wbjp run --live` は、建玉の照会がエラーなしで 0 件なのに台帳では保有中のはず（前の回の建玉の株数に
  その後の買いを足し売りの約定を引いて正・保存済みのストップ）の銘柄があると止まる（`wbjp.positions_empty`）。照会の不調を信じて
  ストップを消し、保有中の銘柄を買い直さないため。手で全部売った後など口座が本当に空なら、
  確かめたうえで `--accept-flat` を付けて 1 回走らせる。**`--accept-flat` は cron の行には書かない**
  （手で 1 回だけ打つ）。付けたままでも 1 回で失効する: 素通りした回が成功すると、同じ日の
  2 回目と、その次の回（間に通常の検査で成功した回が無い）では使えず、通常どおり止まる
...
**寄る前の `snap` は 1 回**（crontab の `DT_PRESNAP_AT`、いま 08:59:40.0・`--slot 085940`、`--max-run 5` / 打ち切り 8 秒 / KILL 1 秒。
2026-09-24 に 8:59:35・8:59:55 の 2 回からまとめた。スロット名は時刻に合わせて変える）。寄る前の `open` の**前**に全銘柄（実測 起動 1.8 秒 + 取得 3.3〜3.6 秒）を
取り切り、固まっても `DT_PRESNAP_AT` + 9 秒（いま 8:59:49）にはロックを手放す——`DT_PRESNAP_AT + 打ち切り + KILL ≤ DT_PREOPEN_AT − 1 秒` を組で保つ。
ロックを待たず（待ち 0 秒）、前の回が握っていれば何もせず終わる——だから `morning-check.sh` の必須スロットには入れていない。
...
当時の 8:59:35 の回が黙って落ちて `open` の余裕も削った）。
...
`daytrade snap`（8:30・8:45・8:55・8:57・8:59:00・寄る前（`DT_PRESNAP_AT`）／9:00:05・9:02・9:05・9:11／15:00・15:19）は板・気配を記録するだけで
...
ふだんの実行は長くても 10 秒。過ぎたら TERM、さらに 10 秒（`WITH_LOCK_KILL_AFTER`。寄る前の `snap` は 1 秒）で KILL し、`[error] [timeout]` をログに残して 124 で終わる。
...
**既定（170 + 10）のままでよいのは、TERM と KILL が寄る前の寄成（`DT_PREOPEN_AT`）と寄る前の snap（`DT_PRESNAP_AT`）より手前に着く行だけ。** 8:30・8:45・8:55 の `snap` は
