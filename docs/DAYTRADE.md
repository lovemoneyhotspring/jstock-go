...
当日決算予定なし・日々公表信用残の対象でない・直近の本決算が赤字でない」銘柄を母集団にし、9:00 の気配で
...
- IV ゲート（`regime.iv_gate`）は前日の日経 225 オプション `BaseVol` 中央値。オプションの足は夜遅く（0 時ごろ）に乗るので
  ...
  取り込みは 0 時を過ぎてから取りに行き、0 行なら 50 分おきに取り直す（2026-09-16。cron は 30 分おきなので、
  ここを 1 時間ちょうどにすると「1 時間後」の回が数秒足りずに落ちて実効 90 分になる——2026-09-17 に 50 分へ。それまでは 0 行を掴むと
  20 時間待って翌日の昼に入り、朝の open に前日ぶんが間に合っていなかった）
...
確定済みまで聞くと、14 暦日ぶんで 1 実行に百を超える電文になり、9:01 の発注より前にそれを消費する。
...
9:00:03 から気配を取ってザラ場の成行で建てる形では、寄っている銘柄（ロング候補の 89.4%）は
寄付の直後の値を追いかけて買い、板を食う分だけ負ける（板の記録の実測で 49.1 bp、実約定 16 件の
中央値 67.7 bp）。寄る前（8:59:45）の気配で並べて**寄成**を出せば、全銘柄が始値で建つ。
...
届く寄成は、既に寄った銘柄の板寄せに間に合わず約定しない。送れなかった分は 9:00:03 からの 7 回が
...
1. **`sCondition ≠ 0` は実機未検証**（[BROKER_VERIFY.md](BROKER_VERIFY.md)）。電文が拒否されれば
   台帳に REJECTED が残り、9:00:03 の回が従来の成行で建て直す（失敗として通知も出る）
2. **寄らなかったときにいつ失効するか**（前場引けか、大引けか、後場の寄付に参加するか）。
   ここが分かるまで「寄らない銘柄の枠をいつ諦めるか」は決められない
3. 8:59:45 の気配と始値の誤差。事前登録では「20 営業日溜めてから測り直して本番」としていたが、
...
（`結果.csv` で 1 つの subject に別物が混ざらないように）。8:59:45 の気配と始値の誤差は
`backtest` には入っていないので、寄成の側は上限の数字。

#### 並べ方を常時 LightGBM に戻した（2026-09-19）

寄る前に出すなら `lgbm` の弱点が消えるため。`lgbm` が選ぶ銘柄は 65% が 9:00:00 に寄っており
（`gap_vol` は 34%）、ザラ場の成行ではその滑りをほぼ全候補で払っていた。滑りを除いた上限
（全銘柄を始値で建てる）は **`lgbm` +37.07 対 `gap_vol` +32.08 bp/日**で `lgbm` が上。

8:59:00 の誤差込みの実測は +9.02 対 +9.18 とほぼ同値で、これは `lgbm` の優位（+5 bp）を気配の
誤差がちょうど食い潰した形。低下幅は `lgbm` −28.05 に対し `gap_vol` −22.90 で、**微細な順位に
依存する `lgbm` のほうが気配の誤差に弱い**。8:59:45 の誤差は 8:59:00 より小さいはずで、それを
測るのがこれから。誤差が縮まらなければ `gap_vol` に戻す判断もありうる。
...
| | `spread_bp_open` / `spread_bp_close` | 成行が払うスプレッドの片道（bp、既定 27 / 11）。**`backtest` と `evaluate` の費用にだけ効く。発注は変えない。** 成行買いは最良売気配、成行売りは最良買気配で約定するので中値との差だけ必ず負けるが、建値に使う分足・ティックは「約定値」なので中値の近くに来ており、この差がどの検証にも入っていなかった。寄付のぶんは**既に寄っている銘柄にだけ**掛かる（未寄付は板寄せに参加して寄値で約定する。板の記録の実測でも食い上がりは 0 bp）。寄ったかどうかが分からない日（分足の無い 2024-09 より前、`evaluate` の順位表）は払う側に倒す。**寄成で建てた脚は寄った銘柄でも払わない**（板寄せで約定するので最良気配を食わない）——ただし判定の材料が `backtest` と `evaluate` で違う。`backtest` は設定（`preopen_legs`）を全期間に当てる「もし寄る前に出していたら」の想定値で、`evaluate` は**その日の台帳の `condition` 列**を見るので遡らない（下記）。実測は下記 |
