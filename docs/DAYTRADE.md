...
- **9:00 以降の回は両脚とも休む**（`usLowPreopenOnly`。見送りの順位表を積む）。寄る前の回が走らなかった・前夜の米国の値が
  間に合わなかった（`us_stale`）・寄指が拒否された朝は、その日は従来どおり休みになる。成行では埋めない
...
- 設定の検証: `preopen_legs` に long が無い・`us_skip_legs` が `short` でないと起動時に止まる（黙って効かない設定にしない）
...
- 戻すとき: `preopen_limit_pct_us_low` をコメントに・`us_skip_legs = "all"`・`rank_by_us_low = ""` の 3 つ（bin の作り直しは不要）
...
| | `preopen_limit_pct_us_low` | **前夜の米国市場が小幅高の日だけ**使う寄指の位置（%、本番 1.5、上限 5、`0` = 使わない）。正なら、この日は寄る前の回のロングを寄指で出すだけで、9:00 以降の回は両脚とも休む。`preopen_legs` に long・`us_skip_legs = "short"` でないと弾く。上の「米国小幅高の日だけ寄指で出す」 |
