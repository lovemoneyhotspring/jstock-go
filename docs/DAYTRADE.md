...
| 9:20・10:20・13:20 | `daytrade protect --live --yes` | **保険の手仕舞い。** 建てた玉のうち約定したぶんに、執行条件「引け」の返済・売りをブローカーへ先に置く（`execution.protect_exit`、既定 `false`＝実機で未検証。何度回しても重ならない）。注文は立花に残るので、cron・マシン・ネットが止まっても引けで手仕舞われる。引け値は 15:20 の成行より不利なので、ふだんは使わず close が取り消す | 台帳 + ブローカー |
| 15:20〜15:30 | `daytrade close --live --yes` | 台帳の当日買いをブローカーに照会し、**生きている保険の引け注文を取り消し（確かめられなければ成行を出さず保険に任せる）**、**まだ板に残っている建て注文（寄らないままの寄成・一部約定の残り）を取り消してから**、確定した約定数量を成行売り。取消の完了を確かめられないときは分かっている約定分を売り、人に知らせる（次の回が増えた分を拾う）（15:20 はその場で約定。15:25 以降はクロージング・オークションで引け値） | 台帳 + ブローカー |
