...
**まだ `none`（本番では使っていない）。** 実装だけ入れてある。

今の形は 9:00:03 から気配を取って並べ、ザラ場の成行で建てる。寄っている銘柄（ロング候補の
89.4%）は寄付の直後の値を追いかけて買い、板を食う分だけ負ける（板の記録の実測で 49.1 bp、
実約定 16 件の中央値 67.7 bp）。寄る前（8:59:45）の気配で並べて**寄成**を出せば、全銘柄が
始値で建つ。
...
1. **`sCondition ≠ 0` は実機未検証**（[BROKER_VERIFY.md](BROKER_VERIFY.md)）。デモ環境で電文が通るかと、
   **寄らなかったときにいつ失効するか**（前場引けか、大引けか、後場の寄付に参加するか）を確かめる。
   ここが分かるまで「寄らない銘柄の枠をいつ諦めるか」は決められない
2. 8:59:45 の気配の記録が 20 営業日（2026-09-18〜）溜まってから、その誤差で測り直す（事前登録）
3. `entry_window` を `["08:59", "09:15"]` にして、cron に 8:59:45 の回を足す（[DEPLOY.md](DEPLOY.md)）
...
| | `preopen_legs` | **寄る前の回で寄成（執行条件「寄付」）を出す脚**。`none`（既定・本番）/ `long` / `short` / `both`。寄成はその銘柄の始値を決める板寄せに参加するので、寄った後の値を追わず始値で建つ。`none` 以外にするなら `entry_window` の開始を 9:00 より前にする（設定の検査で止まる）。9:00 以降の回は脚によらず従来のザラ場の成行。下記「寄る前に出す」 |
