...
| | `spread_bp_open` / `spread_bp_close` | 成行が払うスプレッドの片道（bp、既定 27 / 11）。**`backtest` と `evaluate` の費用にだけ効く。発注は変えない。** 成行買いは最良売気配、成行売りは最良買気配で約定するので中値との差だけ必ず負けるが、建値に使う分足・ティックは「約定値」なので中値の近くに来ており、この差がどの検証にも入っていなかった。寄付のぶんは**既に寄っている銘柄にだけ**掛かる（未寄付は板寄せに参加して寄値で約定する。板の記録の実測でも食い上がりは 0 bp）。寄ったかどうかが分からない日（分足の無い 2024-09 より前、`evaluate` の順位表）は払う側に倒す。実測は下記 |
