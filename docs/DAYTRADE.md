...
| 15:20〜15:30 | `daytrade close --live --yes` | 台帳の当日買いをブローカーに照会し、**まだ板に残っている建て注文（寄らないままの寄成・一部約定の残り）を取り消してから**、確定した約定数量を成行売り。取消の完了を確かめられないときは分かっている約定分を売り、人に知らせる（次の回が増えた分を拾う）（15:20 はその場で約定。15:25 以降はクロージング・オークションで引け値） | 台帳 + ブローカー |
