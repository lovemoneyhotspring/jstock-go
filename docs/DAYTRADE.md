...
- `open` は台帳に当日の建玉（dry-run 以外で、生きている／約定したもの）を数え、**N − その数**だけ建てる（冪等。全部建っていれば何もしない）。前の回が途中で落ちていれば（通信エラー・締め切り）残りを別の銘柄で埋め、同じ銘柄は重ねて建てない。注文 ID は日付・銘柄・数量から決まる。拒否されていれば次の回（9:01:03・9:03〜9:13）で種を変えて送り直す。
