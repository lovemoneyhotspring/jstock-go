...
`open` と `close` は冒頭で、直近 7 暦日の台帳を遡って**手仕舞えていない建玉**を探す。残り株数は
「建玉の約定 − 手仕舞いの約定」をブローカーに照会して出し、ブローカーの建玉と突き合わせる
（建玉が無ければ手で返済済みとみなして対象外。照会できない注文がある銘柄は数量を推測せず、
通知して対象外）。見つかれば新規に建てる前に成行で手仕舞う——`open` なら寄付（板寄せに参加）、
...
- **拘束資金**: 持ち越しの残り株数 × 建値を脚ごとに合計し（保証金は 3 割でも評価損を抱えるので全額を拘束とみなす）、
  残り `max_capital − 拘束` で建てられる件数に減らす（`execute.CapByTied`）。残りが 1 注文の予算に満たなくても
...
- 建玉を照会できない朝は `open` を止める（持ち越しを知らずに建てると二重になりうる）
