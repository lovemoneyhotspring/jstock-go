...
## 信用売りの脚 `jp_gap_fade_margin`（`config/daytrade_margin`、バックテストのみ）

立花証券 e支店（信用手数料 0 円）で、ロングに**信用売り**を足す。ショートはロングの鏡像:
貸借銘柄（`Mrgn == 2`）× 同じ除外 × ギャップ **+3% 以上**の上位 N を寄付で新規売り、大引で買い戻す。
資金配分は「シーソー」——ロング側の資産曲線ゲート（`equity_curve_days`）が効いた日
（＝地合いが弱い日）だけショートを建てる（`margin.multiplier_normal = 0`、`multiplier_long_weak = 1.0`）。
ロング縮小日のショート平均は通常日の 6 倍で、2018・2021 年のようにロングが稼げない年を埋める。
数字は研究ノート「信用売りでの下げ相場対策」。`daytrade backtest --config-dir config/daytrade_margin`。

**発注側は未実装。** `open` / `close` はロングしか出さない。必要なのは (1) 立花証券の信用新規売り・返済買いの
電文、(2) 台帳の売買区分、(3) `verify` の建玉突き合わせ、(4) 51 単元以上を指値に落とす、(5) 売り禁銘柄の除外。
