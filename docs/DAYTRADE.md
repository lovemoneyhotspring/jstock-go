...
下げはショートが受け止めるので、買いは縮めずに利益を追う。合図そのものはショートのために計算し続ける。
信用買いなので建玉は現金より大きくでき、既定はロング 300 万円（現金 200 万・保証金率 33%）。
2017–2026 では現行相当 929 万 / Sharpe 2.35 / DD −58 万 → 1,359 万 / 2.33 / −49 万（滑り 5 bp）。
数字は研究ノート「信用売りでの下げ相場対策」「ロングの DD 抑制を外す」。
`daytrade backtest --config-dir config/daytrade_margin`。

**発注側（2026-09-02）**: `[margin]` が有効なら `open` はロングを信用買い（`long_via_margin`）、
ショートを貸借銘柄（plan の `shortable`）の信用新規売りで出し、`close` は反対売買の返済
...
残る未対応: 売り禁（新規売建停止）銘柄の事前除外（発注時のエラー 11149/11325 で弾かれ、次の回で
再送されない＝機会損失のみ）、51 単元以上の指値への落とし込み（今は拒否）。
