...
```bash
bin/rate sync                       # 立花 API から過去 90 日のレーティングの動きを取り込む（直近 3 日は済みでも取り直す）
bin/rate sync --recent 5            # 済みでも取り直す直近の日数を変える（--force で全部）
...
以前はそのまま二度と取りに行かなかった（`rate.SyncNews`）。

`history` は 1 か月ごとに 5 秒空ける。短すぎると 429 を返される（1.5 秒で止められた）。
429・5xx・接続の失敗（途中で切れたものを含む）は 30 秒から倍々に、0.5〜1.5 倍の揺らぎを入れて待って
取り直す（`rate.FetchWithRetry`、5 回まで）。404 などはすぐ失敗にする。取り直しても駄目な月で止まるが、
それまでに済んだ月は残るので再実行すれば続きから進む。行を読めない月は止めずに次へ進み、取り込み済みにはしない
（`rate.ImportMonths`）。
