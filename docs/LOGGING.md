...
| `daytrade.rule_compare` | LightGBM で並べた日の、ロングの実際の選定と既存規則（gap_vol）の選定の比べ（evaluate の後） | `day`, `lgbm_picks`, `lgbm_avg_net_bp`, `lgbm_hypo_pnl`, `rule_picks`, `rule_avg_net_bp`, `rule_hypo_pnl`, `overlap`, `skipped` |
