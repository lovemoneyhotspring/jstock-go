...
2026-01 に追加された有料アドオン（Light 以上、月額）で株価の分足とティックが取れる。分足は 2026-09-05、ティックは 2026-09-07 に過去 2 年ぶんを入れた。ティックは「ティック（アドオン）」の節。
...
| 全期間（2024-09-02〜2026-09-04、490 営業日、全時間帯） | 18.4 億行 / Parquet 17GB（1 日平均 375 万行・35MB。2026-09-07 に取り込み） |
| **同、09:00〜09:31 に刈った後（2026-09-08）** | **4.15 億行 / 3.5GB**（1 日平均 85 万行・7MB） |
| 1 年の見込み（窓あり） | 約 1.8GB |

設計時（2026-09-03）の「1 日数千万約定、年 50〜100GB」は過大だった。全銘柄・全時間帯でも 2 年 17GB で、
サーバーの空き（取り込み後 46GB、2026-09-07）に入る。**`_raw` は残さない**（`Endpoint.NoRaw`）。csv.gz が Parquet と
...
`check` / `repair` の判定（2026-09-14 に足したもの）:
...
- **ほぼ毎営業日行があるが、本当に 0 行の日もある端点**（日々公表銘柄 `margin-alert`・決算短信 `fins/summary`。
  ...
  ただし**大納会（取引カレンダーで年の最後の営業日）の 0 行は、猶予の内でも欠けにしない**。決算短信は大納会に
  0 行の年が多く（2019・2022・2025 の 12-30）、猶予（2 日）が明けるまで 12/30・12/31 の 20:00 の `repair` が
  毎年終了コード 3 で通知し、夜間自己修復も起きていたため。50 分おきの取り直しは変えない（公開の遅れで空を
  掴んだ大納会も取りに行く。通知しないだけ）。カレンダーがその年の 12-31 まで無ければ大納会と決めず、欠けのまま数える。
...
- **窓を狭めて取り直しても、既存の窓の外は消さない**。日分割は普段その日のファイルを丸ごと差し替えるが、窓が
  あるときは既存ファイルの窓の外の行を読んで残し、窓の中だけ新しい塊で差し替える（`Archive.UpsertKeeping`）。
  以前は窓を狭めたあとに `backfill` / `repair` すると、全時間帯で溜めた日が黙って窓の中だけになっていた。
  窓の外を消すのは `jquants prune` だけ。刈った後の日は窓の外が無いので、読み直しは窓の中の小さなファイルで済む。
- **溜めたぶんは `jquants prune` で刈る**。日分割のファイルを 1 日ずつ読み、窓の外の行を落として書き戻す
  （落とす行は `Frame` に載せないので、常駐は残す行ぶんだけ）。**既定は数えるだけ**で、書き換えるのは
  `--yes` を付けたときだけ（`--dry-run` を付ければ `--yes` があっても数えるだけ）。
  ファイルごとに端点のロックを取ってから読むので、同時に走る `sync` の書き込みを古い中身で上書きしない。
...
```bash
# 分析が終わったら: まず数える（--yes が無ければ数えるだけ）
JQUANTS_TICKS_WINDOWS="09:00-09:31" jquants prune --only equities_trades
# 刈る（cron の sync の行にも同じ環境変数を足して、以後の日次を同じ窓にする）
JQUANTS_TICKS_WINDOWS="09:00-09:31" jquants prune --only equities_trades --yes
```

### 取り込みの時間とメモリ（2026-09-07 の実測）

月次の一括 1 本は csv.gz 1GB・約 1 億行。`BulkDownload` が csv.gz を丸ごとメモリに置き（1GB）、
`CSVToFramesByDay` が日ごとに区切って `Upsert` する。

| | 実測（2026-08 の月次 1 本 + 当月の日次 4 本、1.17 億行・24 日） |
|---|---|
| 所要時間 | 8 分 43 秒（1 日 30〜40 秒） |
