...
| 発注と照会の**合計 10 回/秒**（`orderLimiter` 8 + `requestLimiter` 2）が上限に当たらないか | 立花の上限は 10 回/秒。2026-09-25 に注文の枠を 2 → 8 回/秒へ上げた（2 回/秒のときは 9/20〜9/24 とも `broker.request_failed` なし）。上げた後の最初の寄成で注文の `p_errno`・`sResultCode` が 0 以外にならないか、送信の間隔が約 0.13 秒に詰まったかを見る | 同 `requestLimiterFor` |
