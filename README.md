...
### 2. 損切りはエンジン側で計算する。逆指値はブローカーにも置ける

立花証券 e支店 API は逆指値（`sGyakusasiOrderType` 1）と通常＋逆指値（2）を受け付け、
未発火なら訂正（`CLMKabuCorrectOrder`）で条件価格を変えられる。期日は最長 10 営業日。
`domain.OrderRequest.WithStop` で付け、`broker.StopCorrector` で訂正する。トレーリングは
「逆指値を置いて、定期的に訂正で条件を引き上げる」形で作る（実機での検証は `docs/BROKER_VERIFY.md`）。

→ 今のところ **ストップ価格の計算と判定はエンジン側**（`wbjp.risk.stops`）で、
ローカルに保持して日足更新時に評価し、成行/指値の決済注文を出す。
...
**分足**（有料アドオン）も同じ経路で溜める。履歴が 2 年しか無いので、過去 2 年ぶんを 2026-09-05 に取り込んだ（1 日 47 万行・4.7MB、`JQUANTS_MINUTE_BARS=1` で日次に載る）。**ティック**（同じアドオン）も 2026-09-07 に過去 2 年ぶんを取り込み、検証のうえ 09:00〜09:31 だけ残している（1 日 85 万行・7MB、2 年で 3.5GB。`JQUANTS_TICKS=1`、一括ファイル専用）。**板**はどこにも履歴が無く（立花のリアルタイムだけ）、記録を始めた日からしか手に入らない。何のために集めるかは [docs/OPENING_DATA.md](docs/OPENING_DATA.md)。
...
- **`cd` が要る。** `config/` `data/` `.env` はカレントディレクトリ基準。
  ...
  絶対パス指定できる（そのとき `state` `data` `config` の相対パスは
  **その `.env` のある場所**から解く。リポジトリの外に置くなら絶対パスで書く）
