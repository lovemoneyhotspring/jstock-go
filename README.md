...
`run` の規則はバックテストと同じ: **判断は前日までの確定足、買うのは当日の価格**。入金日 D の投下は D の足が確定した翌営業日に、その日の価格で買う（ザラ場中の途中経過の足では判定しない）。直近 `catch_up_days`（既定 7 日）以内の未発注の投下は繰り越すので、入金日に cron が止まっていても次の実行で買う。最終足が `max_stale_days`（既定 4 日）より古い銘柄は判定せず通知する。発注前にブローカーの見積りと買付余力を突き合わせ、足りなければ見送って通知する。株数は `投下額 ÷ 価格` を単元に切り捨て。ETF は単元が 1 株・10 株のものが多いので `[execution] lot_size_overrides` で指定する（既定の 100 株だと月の予算が届かず丸ごと見送りになる）。注文 ID は日付・銘柄・株数から決定論的に作るため、cron が二重に走っても二重買付にならない。
