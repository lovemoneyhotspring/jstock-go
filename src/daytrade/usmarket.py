...
取得元は yfinance（``^GSPC`` / ``^VIX``、日足）。取れなければ None を返し、ゲートは効かない。
...
def _download(start: dt.date, end: dt.date) -> pl.DataFrame:
    ...
    for symbol, name in SYMBOLS.items():
        ...
        frame = pl.from_pandas(raw.reset_index()[["Date", "Close"]]).select(
            pl.col("Date").cast(pl.Date), pl.col("Close").cast(pl.Float64).alias(name)
        )
        frames.append(frame)
    if not frames:
