...
していない。米国株では使えるが、日本株では使えない。つまり
「ここまで下がったら売る」という注文をブローカーに預けられない。

そこでストップ価格をローカルに保持し、足が更新されるたびに評価して、
抵触していれば決済注文を組み立てる。

**必ず理解しておくべき限界**

日足で運用する以上、判定は1日1回しかできない。場中に急落しても、
その日のうちには反応できず、翌営業日の寄付で手仕舞うことになる。
想定したストップ価格より大きく下で約定する可能性がある。

これは実装の手抜きではなく、
「日本株の逆指値が API に無い」×「日足で運用する」から必然的に
生じる制約。許容できないなら、分足運用とリアルタイムのデータ源が要る。
"""
