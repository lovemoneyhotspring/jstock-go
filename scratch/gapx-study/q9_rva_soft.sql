...
th AS (SELECT quantile_cont(f_rva, 0.2) lo, quantile_cont(f_rva, 0.8) hi FROM c WHERE dd < DATE '2022-01-01'),
...
swap AS (
  SELECT a.dd, (a.f_rva <= th.lo AND b.f_rva > th.hi) sw
  FROM t a JOIN t b ON b.dd = a.dd AND b.pr = 4 CROSS JOIN th WHERE a.pr = 3
),
picks AS (
  SELECT 'base' v, dd, net, 1.0 w FROM t WHERE pr <= 3
  UNION ALL
  SELECT 'R3', t.dd, net, 1.0 FROM t LEFT JOIN swap USING (dd)
    WHERE pr <= 2 OR (pr = 3 AND NOT coalesce(sw, false)) OR (pr = 4 AND coalesce(sw, false))
  UNION ALL
  SELECT 'R4', dd, net, CASE WHEN f_rva > th.hi THEN 1.5 WHEN f_rva <= th.lo THEN 0.5 ELSE 1.0 END FROM t CROSS JOIN th WHERE pr <= 3
),
daily AS (SELECT v, dd, sum(w * net) / sum(w) m FROM picks GROUP BY 1, 2),
