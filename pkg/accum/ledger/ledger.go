...
//
// なぜ取消・失効を「出した」と数えるか（daytrade とは逆）: accum の ID は日付・銘柄・売買・
// 数量だけで決まり（MakeClientOrderID。run・verify-order とも）、再送の種を変える仕組みが無い。
// 同じ ID で送り直すと Record の UPSERT が既存の行を上書きし、ブローカーには 2 件あるのに台帳は
// 1 行になる（2026-09-11 の検証で約定済みの行が PENDING に巻き戻った）。取消・失効は一部約定して
// いることもあるが、この判定は状態だけを見るので約定の有無にかかわらず「出した」側に倒す。
// 代わりに約定 0 株で取り消された日の買い直しはしない（二重発注でなく買い漏れの側）。
