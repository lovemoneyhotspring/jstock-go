...
// 送り直しは次の回に回る。ただし daytrade の規則 R（weighting = "turnover"）では、その間に他のロングを
// 建てると LongClosedForToday でその日のロングが締め切られるので、本当に届いていなかった銘柄は
// その日建たない（建て漏れ。二重発注よりは安全側）。2026-09-25 時点で本番の台帳に結果不明は 0 件。
// 締め切りを越えて「届いていないと決まった銘柄だけ」送り直す形は、実例が出てから判断する
// （vault 30-projects/jstock-go-review-20260925.md）。
//
