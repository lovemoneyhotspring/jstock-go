...
//
// dry-run は数えない。拒否・取消・失効・未送信で**約定 0 株のまま**終わった注文（IsDead）も
// 数えない——同じ判断をもう一度送ってよい（再送は呼び出し側が ID の種を変える）。
//
// なぜこの意味で二重発注にならないか（accum・wbjp は取消・失効を「出した」と数えるが、
// daytrade はこれでよい）:
//   - 取消・失効は照会の結果（ブローカーの状態）、未送信は注文一覧との突き合わせ、拒否は
//     送信の応答でしか書かない。どれも終了状態で約定 0 株なら、板に生きた注文も建玉も無い
//   - 送り直しの ID は DeadCount（その日・銘柄・売買の終わった注文の数）を種に変わるので、
//     終わった注文の ID をもう一度使うことは無く、台帳の行も上書きしない
//     （accum・wbjp の ID は日付・銘柄・売買・数量だけで決まり、同じ ID の再送は行を上書きする）
//   - 一部でも約定して終わった注文は「出した」と数える。PlacedToday もその銘柄を建てた側に数え、
//     残りを送り直すときは DeadCount が種を変える
//   - 送信結果不明（PENDING）・UNKNOWN は届いたかもしれないので「出した」側
//
