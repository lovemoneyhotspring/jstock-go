...
// （broker.requestLimiter は注文・照会の合計で 8 回/秒。2026-09-25 まで注文だけで 2 回/秒）。開始（crontab の DT_PREOPEN_AT）の約 1 秒後から送り、
// 締め切りの 1 秒前（execute.EntrySendMargin）で止まるので、n 本目は 開始 + 1.0 + (n−1) × 0.13 秒 ≤ 59.0。
// 8:59:52 開始なら約 45 本まで入る。10 本は送信が 2 回/秒・8:59:53 開始（12 本が境目）のときに決めた上限のまま
