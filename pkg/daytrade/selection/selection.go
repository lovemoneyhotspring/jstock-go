...
type Quote struct {
	...
	Opened bool
	// HasBook は**この応答に板（最良気配）が返っている**。板が返っているなら、その値段は
	// 問い合わせた時点のもの——現在値時刻（tDPP:T）が古くても気配は古くない。
	// tDPP:T は「最後に約定した時刻」なので、約定の薄い銘柄は板が生きていても古いと
	// 判定される。寄付が遅れる銘柄＝利益源がそこで構造的に落ちていた（2026-09-12）。
	// 鮮度の判定（quotes.Fresh）はこの印を見る。
	HasBook bool
