"""LightGBM の特徴量（順位 / 生の値）と教師データ（日ごとの順位 / 超過 bp の 5 段階）を分けて入れ替え、どちらが効くかを切り分ける。

根拠: vault 20-research/2026-09-jp-daytrade-raw-feats.md の追記（2026-09-26、ユーザの提案）。生の特徴量＋生の bp は予備ではっきり
悪化し、生の bp では early stopping が木 1 本まで縮んだ。教師データのせいか特徴量のせいかを分け、大きさを段階で残す形も試す。

  PYTHONPATH=test test/.venv/bin/python test/dt_lgbm_target_bins.py --prelim   # 予備（誤差なし＋9/24 までの 0859、採否に使わない）
  PYTHONPATH=test test/.venv/bin/python test/dt_lgbm_target_bins.py --check    # 材料の日数だけ
  PYTHONPATH=test test/.venv/bin/python test/dt_lgbm_target_bins.py            # 判定（材料 20 日から）

事前登録（2026-09-26、結果を見る前に固定。この docstring を commit してから回す）:
  学習・本番の形・気配の誤差・判定の条件 2〜3 は test/dt_lgbm_raw_feats.py と同じ（walk-forward 7 fold・L2 回帰・規則 R ÷7・
  10 位・1,000 万・小幅高の日は寄指 −1.5%・判定は 8:59:52 の層の材料 20 日を day_boot で 20 シード）。
  教師データ:
    順位   寄→引の日ごとの百分位（今）
    5 段階 寄→引からその日の候補の平均を引いた超過 bp を −150 / −50 / +50 / +150 で区切った 0〜4（L2 回帰で 0〜4 を当てる）。
           区切りは最初の fold（2019-09-17）より前の候補の分布で決めた（およそ 9 / 22 / 39 / 22 / 8%）。生の bp のままだと
           日ごとの市場全体の上下が大半を占めて木が育たなかったので、日の平均を引き、日をまたいだ大きさは段階で残す。
  モデル:
    M0   特徴量は順位・教師データは順位（今）
    MR1  特徴量は生の値（dt_lgbm_train.raw_features、±inf は欠け）・教師データは順位
    MR5  特徴量は生の値・教師データは 5 段階
    M5   特徴量は順位・教師データは 5 段階
  判定（主）: 米国小幅高の日の LR1 − L・LR5 − L・L5 − L。3 本なので誤差込みの t ≥ 2.4（5% ÷ 3）かつ差 > 0、誤差なしでも差 > 0、
             前半・後半とも差 > 0 をすべて満たしたものを「効く」。複数なら誤差込みの t の大きい方。どれも満たさなければ今のまま。
...
from dt_lgbm_raw_feats import feats  # noqa: E402
...
T_MIN = 2.4
BIN_EDGES = [-150, -50, 50, 150]      # 超過 bp の区切り（0〜4）
MODELS = {"M0": ("rank", "rank"), "MR1": ("raw", "rank"), "MR5": ("raw", "bin5"), "M5": ("rank", "bin5")}
VARIANTS = {"L": "M0", "LR1": "MR1", "LR5": "MR5", "L5": "M5"}


def target(tr, kind):
    if kind == "rank":
        return tr.groupby("d")["y_raw"].rank(pct=True).values
    ex = (tr["y_raw"] - tr.groupby("d")["y_raw"].transform("mean")).values * 1e4
    return np.digitize(ex, BIN_EDGES).astype(float)
