...
「≤ 10」の水準が見えていなかった。ユーザの提案で、定番の区切りで段にして入れる）
...
事前登録（2026-09-25 夜、結果を見る前に固定。この docstring を commit してから回す）:
  ...
  モデル:
    ...
    MB  M0 ＋ RSI(2) の段（百分位に直さない順序つきの整数 0〜5）。区切りは定番の値 5 / 10 / 20 / 30 / 70:
        0: ≤ 5、1: ≤ 10、2: ≤ 20、3: ≤ 30、4: ≤ 70、5: > 70（欠けは欠けのまま）。区切りは調整しない
  並べ方:
    L    M0 の予測値の高い順
  ...
  判定（主）: 米国小幅高の日で LP2 − L と LB − L（シードで平均した日次の差）。2 本を試すので t ≥ 2.3 かつ差 > 0、
             さらに誤差なし（見えるギャップ = 始値のギャップ）でも同じ日の差が > 0 なら「効く」。
  記述（採否に使わない）: 誤差込みの t、それ以外の日の同じ差、年ごと、段ごとの上位 10 本の占有。
"""
...
def rsi2_bin(v):
    b = np.digitize(v, EDGES, right=True).astype(float)
    b[np.isnan(v)] = np.nan
    return b
...
def feats(g, extra):
    ...
    if extra:
        x = np.column_stack([x, rsi2_bin(g["rsi2"].values)])
    return x
...
def run_once(g, models, gf, rules, kappa=None):
    sc = {}
    for name, extra in (("M0", False), ("MB", True)):
        X = feats(g, extra)
        s = np.empty(len(g))
        for k, m in enumerate(models[name]):
            s[gf == k] = m.predict(X[gf == k])
        sc[name] = s
    by = {"L": cap_and_rank(g, sc["M0"]), "LB": cap_and_rank(g, sc["MB"]), "G": cap_and_rank(g, -g["key_sort"].values)}
...
def main():
    ...
    for k in range(len(FOLD_STARTS)):
        ...
        print(f"{FOLD_STARTS[k]}: 木 M0 {models['M0'][-1].n_estimators}・MB {models['MB'][-1].n_estimators} 本", flush=True)
    ...
    print("| 日 | L | LP2 − L（t／誤差込み／誤差なしの差） | LB − L（同） | G | L − G（t／誤差込み） |")
    print("|---|---|---|---|---|---|")
    for name, m in (("米国小幅高の日（主）", uslow), ("それ以外の日", ~uslow), ("全日", np.ones(len(alld), bool))):
        ...
        print(f"| {name} | {mean['L'][m].mean():,.0f} | {c('LP2', 'L')} | {c('LB', 'L')} | {mean['G'][m].mean():,.0f} | "
              f"{c('L', 'G', False)} |")
    yr = alld.year
    ...
    print("記述: 誤差なしの L の上位 10 本での RSI(2) の段の占有 " +
          "、".join(f"{int(b)}:{(rsi2_bin(top['rsi2'].values) == b).mean():.1%}" for b in range(6)))
    for v in variants:
