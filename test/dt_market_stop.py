...
FROM read_parquet('{f}') WHERE Code IN ('{codes}') AND Date >= '2024-09-02' AND TRY_CAST(Vo AS DOUBLE) > 0
  AND (Code = '13060' OR Time BETWEEN '09:00' AND '09:05' OR Time BETWEEN '09:59' AND '10:05' OR Time BETWEEN '11:29' AND '11:31' OR Time BETWEEN '12:59' AND '13:05' OR Time BETWEEN '15:19' AND '15:30')
""").df())
...
rows=[]
for X,T in itertools.product([-0.01,-0.015,-0.02],['10:00','11:30','13:00']):
...
g=pd.DataFrame(rows); g.to_csv('out/dt_market_stop.csv',index=False)
v=g[g.trades>=20]
...
for X,T in [(-0.01,'10:00'),(-0.015,'11:30'),(-0.02,'13:00')]:
