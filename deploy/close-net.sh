...
#
# 「今日の close が成功した」記録（ダイジェストの command=close・outcome=ok・15:20 JST 以降）が
# あれば何もしない。無ければ close を走らせる——close は何度回しても重ならない（手仕舞い済みは
...
if [ -f "$digest" ]; then
  n=$(jq -c --arg since "${day}T06:20" \
        'select(.command == "close" and .outcome == "ok" and (.verify | not) and (.ts_utc >= $since))' \
...
WITH_LOCK_TIMEOUT=170 "$HOME_DIR/deploy/with-lock.sh" /tmp/daytrade.lock 30 "$HOME_DIR/state/logs/daytrade-close.log" \
  "$HOME_DIR/bin/daytrade" close --config-dir "$CFG" --live --yes
rc=$?
if [ "$rc" -eq 0 ]; then
  # 休場日は close が見送る（digest は skipped）。手仕舞いを実行した日だけ知らせる
  if [ -f "$digest" ] && jq -e 'select(.command == "close" and .outcome == "ok" and (.verify | not) and (.ts_utc >= "'"${day}"'T06:20"))' "$digest" >/dev/null 2>&1; then
