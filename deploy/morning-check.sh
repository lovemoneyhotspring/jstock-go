...
#
# 8:59:35（085935）と 8:59:55（085955）はここに入れない。どちらもロックの待ちが 0 秒で、
# 8:59:45 の open（寄成）が握っていれば**何もせず終わる設計**なので、必須にすると
# 「発注が優先されただけ」の日に毎回誤報が出る。下の集計表には出るので、取れた日は読める。
# 逆に古いスロットを残しても誤報になる——085945 は 2026-09-19 に 085935 / 085955 へ
# 分けたので消した（残すと毎営業日「記録がありません」を出す）。cron を触ったらここも直す
EXPECTED_SLOTS=(0830 0845 0855 0857 0859 0900 0902 0905 0911)
