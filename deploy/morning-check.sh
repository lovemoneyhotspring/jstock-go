...
#
# 8:59:42（085942）はここに入れない。ロックの待ちが 0 秒で、前の回が握っていれば
# **何もせず終わる設計**なので、必須にすると「発注が優先されただけ」の日に誤報が出る。
# 下の集計表には出るので、取れた日は読める。
# 逆に古いスロットを残しても誤報になる——085945 は 2026-09-19 に 085935 / 085955 へ分け、
# 2026-09-24 に 085943 の 1 回へまとめ、2026-09-25 に 085940 へ前倒し・同日 085942 へ（残すと毎営業日「記録がありません」を出す）。cron を触ったらここも直す
