...
回る。`review --days` にはそれ以前のショートの成績が混ざるので、**今のショートの成績としては書かない**。

**ロングは 2026-09-25 から規則 R**（`capital.weighting = "turnover"`）。業種上限後の gap_vol 順に
1 銘柄 min(売買代金 20 日中央値 × 0.2%, 総額 ÷ 7) を 20 位まで割り当てる。**銘柄数は日ごとに変わり（10 前後）、
1 銘柄の金額も揃わない**のが正常。順位表の N は 20・budget は総額 ÷ 20 で、「N 件に満たない」は異常ではない。
総額は朝の建可能額 × 62%（天井 1,000 万、ショック日は ×1.5 の後 72% で頭打ち。2026-09-25 に 68% / 77% から下げた）で毎朝決まる
（ログ `daytrade.margin_cap` の「保証金の比で上限を決め直す」）。1 銘柄が総額の 1/7 を超えていたら異常。
余りの現金（数十万円）が出るのも正常。根拠は vault 20-research/2026-09-jp-daytrade-nscale.md。
